...
onready var TimeCount: Button = $TimeCount
...
func _process(delta):
	if running:
		TimeCount.text = str((OS.get_ticks_msec() - time) / 1000)
...
func summarize(caselist: Array) -> void:
	...
	for case in caselist:
		...
		if case.success:
			...
			failed += 1
	TimeCount.text = time_taken as String
...
func _setup_editor_assets(assets_registry):
	TimeCount.icon = assets_registry.load_asset(Time.icon)
